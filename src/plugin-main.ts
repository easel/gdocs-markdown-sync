...
class ChangeDetector {
  ...
  async detectChanges(file: TFile): Promise<SyncState> {
    ...
    const hasRemoteChanges = await this.plugin.hasRemoteChanges(metadata.id, lastSynced);
...
export default class GoogleDocsSyncPlugin extends Plugin {
  ...
  async onunload() {
    ...
    this.headerActions.forEach((action, fileId) => {
  ...
  updateHeaderAction(leaf: WorkspaceLeaf | null): void {
    ...
    this.updateTimeout = setTimeout(async () => {
      ...
      for (const selector of selectors) {
        ...
        elements.forEach(element => {
          try {
            ...
            if (element.getAttribute('aria-label')?.includes('Google Docs') || 
      ...
      if (leaf?.view?.getViewType() === 'markdown' && leaf.view.file) {
        const syncState = await this.changeDetector.detectChanges(leaf.view.file);
        const headerAction = leaf.view.addAction('sync', 'Google Docs Sync', async (evt) => {
          await this.performSmartSync(leaf.view.file);
        });
        ...
        // Store reference
        this.headerActions.set(leaf.view.file.path, headerAction);
      }
  ...
  updateHeaderActionIcon(action, syncState) {
  ...
  addContextMenuItems(menu, file) {
    ...
    if (metadata) {
      menu.addItem((item) => {
        item.setTitle('Push to Google Docs')
          .setIcon('upload')
          .onClick(() => this.pushSingleFile(file));
      });
      
      menu.addItem((item) => {
        item.setTitle('Pull from Google Docs')
          .setIcon('download')
          .onClick(() => this.pullSingleFile(file));
      });
      
      menu.addItem((item) => {
        item.setTitle('Smart sync with Google Docs')
          .setIcon('sync')
          .onClick(() => this.performSmartSync(file));
      });
    } else {
      menu.addItem((item) => {
        item.setTitle('Create Google Doc')
          .setIcon('file-plus')
  ...
  showStatusBarMenu(evt) {
    ...
    menu.addItem((item) => {
      item.setTitle('Push all documents')
        .setIcon('upload')
        .onClick(() => this.pushAllDocs());
    });
    
    menu.addItem((item) => {
      item.setTitle('Pull all documents')
        .setIcon('download')
        .onClick(() => this.pullAllDocs());
    });
    
    menu.addItem((item) => {
      item.setTitle('Smart sync current document')
        .setIcon('sync')
  ...
  async showFileSyncMenu(file, evt) {
    const menu = new Menu();
  ...
  async pushSingleFile(file) {
    ...
    try {
      ...
      notice.setMessage(`Push failed: ${error.message}`);
      setTimeout(() => notice.hide(), 5000);
      console.error('Push failed:', error);
    }
  }

  async pullSingleFile(file) {
    ...
    try {
      ...
      notice.setMessage(`Pull failed: ${error.message}`);
      setTimeout(() => notice.hide(), 5000);
  ...
  async getGoogleDocsMetadata(file) {
  ...
  getGoogleDocsMetadataSync(file) {
  ...
  async updateFileWithNewDocId(file, docId) {
    const content = await this.app.vault.read(file);
  ...
  async createGoogleDocFromFile(file) {
    ...
    try {
      ...
      notice.setMessage(`Failed to create Google Doc: ${error.message}`);
      setTimeout(() => notice.hide(), 5000);
  ...
  async hasRemoteChanges(docId, lastSynced) {
  ...
  async getOrCreateGoogleDriveFolder(folder) {
