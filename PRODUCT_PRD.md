...
- As an Obsidian user, I can click "Start Authentication" in plugin settings to authenticate with Google Drive
- As an Obsidian user, I can complete OAuth in my default browser following the out-of-band flow
- As an Obsidian user, I can copy the authorization code from Google's success page and paste it back into the plugin modal
- As an Obsidian user, I receive clear feedback when authentication succeeds or fails
- As an Obsidian user, I can configure my own OAuth Client ID and Client Secret in plugin settings for organizational use
...
- **Token Refresh**: Automatic token refresh before expiry for both flows
...
- **Asset handling strategy**: Preserve images and embedded content in Docs ↔ Markdown workflows with local asset management
- **Rich formatting preservation**: Improved handling of tables, lists, and advanced formatting during round-trip conversion
- **Comment integration**: Maintain Google Docs comments in Markdown format using structured annotations
- **Attachment management**: Sync linked files and embedded content alongside document text

### Integration & Workflow Features

- **Local file system monitoring**: Watch mode with immediate sync on file changes for responsive editing workflows
- **Multiple vault support**: Obsidian plugin configuration for multiple vault/Drive folder pairs
- **Conflict resolution UI**: Visual diff tool in plugin for manual merge decisions with side-by-side comparison

### Distribution & Accessibility

- **Prebuilt plugin packaging**: Automated plugin distribution through Obsidian Community Plugins marketplace
- **CLI distribution**: NPM registry publication for simplified installation and updates
